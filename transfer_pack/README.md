...
## 5. Releasing GGLasso on conda-forge

Packages are built and published from the feedstock
[`conda-forge/gglasso-feedstock`](https://github.com/conda-forge/gglasso-feedstock).
The workflow per release:

1. Publish the new version to PyPI (tag + sdist).
2. The `regro-cf-autotick-bot` automatically opens a version-bump PR on the
   feedstock (updates `version` + `sha256`, re-renders).
3. **Manual step:** a recipe maintainer reviews the PR, updates
   `recipe/meta.yaml` if dependencies or the build backend changed (the bot only
   bumps version/sha), waits for CI to pass, and merges.
4. On merge, conda-forge CI builds and uploads to the `conda-forge` channel
   automatically — no manual upload.

Latest release PR (v0.3.0):
[conda-forge/gglasso-feedstock#8](https://github.com/conda-forge/gglasso-feedstock/pull/8).
It needed the manual step in (3): v0.3.0 switched the build backend to
`hatchling`, so `recipe/meta.yaml` had `host: setuptools` replaced with
`hatchling`, the build script set to
`pip install . -vv --no-build-isolation`, and `regain` dropped from `run`
(it became an optional `examples` extra upstream).