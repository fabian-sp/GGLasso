...
from .helper.utils import _save_labels
...
class glasso_problem:
    ...
    def __init__(self, S, N, reg="GGL", reg_params=None, latent=False, G=None, do_scaling=False):
        ...
        self.labels = _save_labels(S.copy())
        ...
        if self.multiple:
            assert reg in ["GGL",
                           "FGL"], "Specify 'GGL' for Group Graphical Lasso or 'FGL' for Fused Graphical Lasso (or None for Single Graphical Lasso)"
            self.reg = reg
        else:
            self.reg = None
    ...
    def _derive_problem_formulation(self):
        ...
        assert isinstance(self.N, (
        int, float, np.integer, np.float, np.ndarray)), "N must be either of type float, int or np.ndarray."
        ...
        if type(self.S) == np.ndarray:
            ...
            if len(self.S.shape) == 3:
                ...
                assert len(
                    self.S.shape) == 2, f"The specified covariance data has shape {self.S.shape}, GGLasso can only handle 2 or 3dim-input"
                self.conforming = True
                self._check_covariance_2d()

                assert isinstance(self.N, (
                int, float, np.integer, np.float)), "For SGL problems, N needs to be a single number, float or int."

        elif type(self.S) == list or type(self.S) == dict:

            assert len(
                self.S) > 1, "Covariance data is a list/dict with only one entry. This is a Single Graphical Lasso problem. Specify S as 2d-array."
            ...
            raise TypeError(
                f"Incorrect input type of S. You input {type(self.S)}, but np.ndarray or list/dict is expected.")
    ...
    def _check_covariance_3d(self):
        ...
        assert self.S.shape[1] == self.S.shape[
            2], f"Dimensions are not correct, 2nd and 3rd dimension have to match but shape is {self.S.shape}. Specify covariance data in format(K,p,p)!"
    ...
    def _check_covariance_2d(self):
        ...
        assert self.S.shape[0] == self.S.shape[
            1], f"Dimensions are not correct, 1st and 2nd dimension have to match but shape is {self.S.shape}. Specify covariance data in format(p,p)!"
    ...
    def _check_covariance_list(self):
        ...
        for k in range(self.K):
            assert self.S[k].shape[0] == self.S[k].shape[
                1], f"Dimensions are not correct, 1st and 2nd dimension have to match but do not match for instance {k}."
            assert np.max(
                np.abs(self.S[k] - self.S[k].T)) <= assert_tol, f"Covariance data for instance {k} is not symmetric."
    ...
    def _scale_input_to_correlation(self):
        ...
        warnings.warn(
            "NOTE: Input data S is rescaled to correlations, this has impact on the scale of the regularization parameters!")
        warnings.warn(
            "The output/solution is rescaled to covariances. All model selection output, in particular the optimal regularization parameters in self.reg_params are corresponding to the correlations.")
    ...
    def solve(self, Omega_0=None, solver_params=dict(), tol=1e-8, rtol=1e-7, solver='admm', verbose=False):
        ...
        assert self.reg_params.get(
            'lambda1') is not None, "Regularization parameters need to be set first (at least lambda1), see function glasso_problem.set_reg_params()"
        ...
        if not self.multiple:
            ...
            sol, info = ADMM_MGL(S=self.S, lambda1=self.reg_params['lambda1'], lambda2=self.reg_params['lambda2'],
                                 reg=self.reg,
            ...
            sol, info = ext_ADMM_MGL(S=self.S, lambda1=self.reg_params['lambda1'], lambda2=self.reg_params['lambda2'],
                                     reg=self.reg,
        ...
        sol['labels'] = self.labels
        # set the computed solution
        if self.latent:
            self.solution._set_solution(Theta=sol['Theta'], labels=sol['labels'], L=sol['L'])
        else:
            self.solution._set_solution(Theta=sol['Theta'], labels=sol['labels'])
    ...
    def set_modelselect_params(self, modelselect_params=None):
        ...
        if modelselect_params is None:
            modelselect_params = dict()
            warnings.warn(
                "No grid for model selection is specified and thus default (or previous) values are used. A grid can be specified with the argument modelselect_params.")
    ...
    def model_selection(self, modelselect_params=None, method='eBIC', gamma=0.1, tol=1e-7, rtol=1e-7, store_all=False):
        ...
        * MGL and ``latent=True``: in a first stage, solve SGL on a :math:`(\\lambda_1, \\mu_1)` for each instance :math:`k=1,\\dots,K` independently. Then, do a grid search on :math:`(\\lambda_1, \\lambda_2)` values and for each :math:`\\lambda_1` and each instance :math:`k=1,\\dots,K` pick the :math:`\\mu_1` value which had minimal eBIC in stage one. Then, pick again the grid point with minimal eBIC.
        ...
        if not np.all(
                self.modelselect_params['lambda1_range'] == np.sort(self.modelselect_params['lambda1_range'])[::-1]):
            warnings.warn(
                "Ideally the lambda1 range is sorted in descending order, so the grid search is performed from sparse to dense.")

        if store_all:
            warnings.warn(
                "Setting store_all=True might cause memory issues as the solution is stored at all grid points.")

        if self.do_scaling and (self.modelselect_params['lambda1_range'].max() > 1):
            warnings.warn(
                "Using do_scaling=True, you can restrict the range for lambda1 to 1. Larger lambdas will result in the zero solution.")
        ...
        if not self.multiple:
            sol, self._all_theta, self._all_lowrank, stats = single_grid_search(S=self.S,
                                                                                lambda_range=self.modelselect_params[
                                                                                    'lambda1_range'], N=self.N, \
                                                                                method=method, gamma=gamma,
                                                                                latent=self.latent,
                                                                                mu_range=self.modelselect_params[
                                                                                    'mu1_range'],
                                                                                use_block=True, store_all=store_all,
                                                                                tol=tol, rtol=rtol,
                                                                                lambda1_mask=self.modelselect_params[
                                                                                    'lambda1_mask'])
            ...
            ###############################
            if self.latent:
                self._est_uniform, self._est_indv, stage1_statistics = K_single_grid(S=self.S, lambda_range=
                self.modelselect_params['lambda1_range'], N=self.N, method=method, \
                                                                                     gamma=gamma, latent=self.latent,
                                                                                     mu_range=self.modelselect_params[
                                                                                         'mu1_range'],
                                                                                     use_block=True,
                                                                                     store_all=store_all, tol=tol,
                                                                                     rtol=rtol)
            ...
            stats, best_ix, sol = grid_search(solver, S=self.S, N=self.N, p=self.p, reg=self.reg,
                                              l1=self.modelselect_params['lambda1_range'], \
                                              l2=self.modelselect_params['lambda2_range'], w2=None, method=method,
                                              gamma=gamma, \
                                              G=self.G, latent=self.latent,
                                              mu_range=self.modelselect_params['mu1_range'], ix_mu=ix_mu, \
...
class GGLassoEstimator():
    ...
    def _set_solution(self, Theta, labels, L=None):
